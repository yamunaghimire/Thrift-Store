...
//               </p>
//               <Link to="/checkout"><button className="bg-pink-500 text-white w-full py-2 rounded-md mt-4">
//                 Checkout
//               </button>
//               </Link>
//             </div>
...
const ProductDetails = () => {
  ...
  const handleAddToCart = () => {
    ...
    if (!product || product.sold) return; // Prevent adding sold-out products
    addToCart(product);
    setCartVisible(true);
  };
  ...
  return (
    <div className="relative p-[180px] grid grid-cols-2">
      ...
      <div className="mt-10">
        ...
        <div className="mt-6">
          <button
            className={`px-4 py-2 rounded-md w-[200px] ${
              product.sold
                ? "bg-gray-400 cursor-not-allowed"
                : "bg-pink-500 text-white"
            }`}
            onClick={handleAddToCart}
            disabled={product.sold || isProductInCart} // Disable if sold or already in cart
          >
            {product.sold
              ? "Sold Out"
              : isProductInCart
      ...
      {/* Cart Layout */}
      <div
        className={`fixed top-0 right-0 h-full w-[300px] bg-white shadow-lg z-50 transition-transform duration-300 ${
