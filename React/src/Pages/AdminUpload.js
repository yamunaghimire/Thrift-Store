...
//       payload.append("price", parseFloat(formData.price)); // Ensure price is a number
...
const AdminUpload = () => {
  ...
  useEffect(() => {
    ...
    const fetchCategories = async () => {
      try {
        ...
        if (response.ok) {
          setCategories(result); // Assuming the response contains an array of categories
        } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      payload.append("category", formData.category); // Send the selected category ID
      payload.append("image", image);
